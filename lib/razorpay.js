...
const resolveRazorpayCredentials = (overrides = {}) => {
  const keyId = toTrimmedString(overrides?.keyId || overrides?.key_id || process.env.RAZORPAY_KEY_ID);
  const keySecret = toTrimmedString(
    overrides?.keySecret || overrides?.key_secret || process.env.RAZORPAY_KEY_SECRET
  );
  return { keyId, keySecret };
};

const assertRazorpayCredentials = (overrides = {}) => {
  const { keyId, keySecret } = resolveRazorpayCredentials(overrides);
  if (!keyId || !keySecret) {
...
const getRazorpayAuthHeader = (overrides = {}) => {
  const { keyId, keySecret } = assertRazorpayCredentials(overrides);
...
export const isRazorpayConfigured = (overrides = {}) => {
  const { keyId, keySecret } = resolveRazorpayCredentials(overrides);
...
export const fetchRazorpayPaymentLink = async (paymentLinkId, { credentials = {} } = {}) => {
...
export const fetchRazorpayPaymentLinkPayments = async (
  paymentLinkId,
  { credentials = {}, ...options } = {}
) => {
  const id = toTrimmedString(paymentLinkId);
...
export const verifyRazorpayPaymentLink = async ({
  ...
  const link = await fetchRazorpayPaymentLink(normalizedLinkId, { credentials });

  let payments = [];
  let paymentsLookupError = null;
  try {
    const paymentList = await fetchRazorpayPaymentLinkPayments(normalizedLinkId, {
      count: 20,
      skip: 0,
      credentials,
    });
