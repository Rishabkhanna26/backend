...
const WHATSAPP_RECOVERY_SETTING_DEFAULTS = Object.freeze({
  pendingRecoveryEnabled: true,
  onlyPostConfigMessages: true,
  preconfigMessageGraceMs: 30000,
  recoveryWindowHours: 24,
  recoveryBatchLimit: 20,
  recoveryAnalysisMessageLimit: 18,
  recoveryAiRequired: true,
  recoveryAiModel: null,
});
...
const normalizeRecoveryPreconfigGraceMs = (value) => {
  const num = normalizeInteger(value);
  if (num === null || num < 0 || num > 300000) return null;
  return num;
};

const normalizeRecoveryWindowHours = (value) => {
  const num = normalizeInteger(value);
  if (num === null || num < 1 || num > 168) return null;
  return num;
};

const normalizeRecoveryBatchLimit = (value) => {
  const num = normalizeInteger(value);
  if (num === null || num < 1 || num > 200) return null;
  return num;
};

const normalizeRecoveryAnalysisMessageLimit = (value) => {
  const num = normalizeInteger(value);
  if (num === null || num < 6 || num > 80) return null;
  return num;
};

const normalizeRecoveryAiModel = (value) => {
...
async function ensureAdminBusinessColumns() {
  ...
  adminBusinessColumnsReadyPromise = (async () => {
    ...
    await poolRef.query(
      `ALTER TABLE admins ADD COLUMN IF NOT EXISTS whatsapp_pending_recovery_enabled BOOLEAN NOT NULL DEFAULT TRUE`
    );
    await poolRef.query(
      `ALTER TABLE admins ADD COLUMN IF NOT EXISTS whatsapp_only_post_config_messages BOOLEAN NOT NULL DEFAULT TRUE`
    );
    await poolRef.query(
      `ALTER TABLE admins ADD COLUMN IF NOT EXISTS whatsapp_preconfig_message_grace_ms INTEGER NOT NULL DEFAULT 30000`
    );
    await poolRef.query(
      `ALTER TABLE admins ADD COLUMN IF NOT EXISTS whatsapp_recovery_window_hours SMALLINT NOT NULL DEFAULT 24`
    );
    await poolRef.query(
      `ALTER TABLE admins ADD COLUMN IF NOT EXISTS whatsapp_recovery_batch_limit SMALLINT NOT NULL DEFAULT 20`
    );
    await poolRef.query(
      `ALTER TABLE admins ADD COLUMN IF NOT EXISTS whatsapp_recovery_analysis_message_limit SMALLINT NOT NULL DEFAULT 18`
    );
    await poolRef.query(
      `ALTER TABLE admins ADD COLUMN IF NOT EXISTS whatsapp_recovery_ai_required BOOLEAN NOT NULL DEFAULT TRUE`
    );
    await poolRef.query(
      `ALTER TABLE admins ADD COLUMN IF NOT EXISTS whatsapp_recovery_ai_model VARCHAR(120)`
    );
    await poolRef.query(
    ...
    await poolRef.query(
      `ALTER TABLE catalog_items ADD COLUMN IF NOT EXISTS is_time_based BOOLEAN NOT NULL DEFAULT FALSE`
    );
    await poolRef.query(
    ...
    await poolRef.query(
      `
        UPDATE admins
        SET
          whatsapp_preconfig_message_grace_ms = CASE
            WHEN whatsapp_preconfig_message_grace_ms BETWEEN 0 AND 300000
              THEN whatsapp_preconfig_message_grace_ms
            ELSE ${WHATSAPP_RECOVERY_SETTING_DEFAULTS.preconfigMessageGraceMs}
          END,
          whatsapp_recovery_window_hours = CASE
            WHEN whatsapp_recovery_window_hours BETWEEN 1 AND 168
              THEN whatsapp_recovery_window_hours
            ELSE ${WHATSAPP_RECOVERY_SETTING_DEFAULTS.recoveryWindowHours}
          END,
          whatsapp_recovery_batch_limit = CASE
            WHEN whatsapp_recovery_batch_limit BETWEEN 1 AND 200
              THEN whatsapp_recovery_batch_limit
            ELSE ${WHATSAPP_RECOVERY_SETTING_DEFAULTS.recoveryBatchLimit}
          END,
          whatsapp_recovery_analysis_message_limit = CASE
            WHEN whatsapp_recovery_analysis_message_limit BETWEEN 6 AND 80
              THEN whatsapp_recovery_analysis_message_limit
            ELSE ${WHATSAPP_RECOVERY_SETTING_DEFAULTS.recoveryAnalysisMessageLimit}
          END,
          whatsapp_pending_recovery_enabled = COALESCE(whatsapp_pending_recovery_enabled, TRUE),
          whatsapp_only_post_config_messages = COALESCE(whatsapp_only_post_config_messages, TRUE),
          whatsapp_recovery_ai_required = COALESCE(whatsapp_recovery_ai_required, TRUE),
          whatsapp_recovery_ai_model = NULLIF(btrim(COALESCE(whatsapp_recovery_ai_model, '')), '')
      `
    );
  })().catch((error) => {
  ...
  try {
    ...
    if (Number.isFinite(Number(adminId)) && Number(adminId) > 0) {
      [rows] = await connection.query(
        ...
        [normalizedOrderId, Number(adminId)]
      );
    } else {
      [rows] = await connection.query(
        `DELETE FROM orders
...
export async function getAdminAISettings(adminId) {
  ...
  try {
    ...
    return {
      ...
      whatsapp_pending_recovery_enabled:
        typeof row.whatsapp_pending_recovery_enabled === 'boolean'
          ? row.whatsapp_pending_recovery_enabled
          : WHATSAPP_RECOVERY_SETTING_DEFAULTS.pendingRecoveryEnabled,
      whatsapp_only_post_config_messages:
        typeof row.whatsapp_only_post_config_messages === 'boolean'
          ? row.whatsapp_only_post_config_messages
          : WHATSAPP_RECOVERY_SETTING_DEFAULTS.onlyPostConfigMessages,
      whatsapp_preconfig_message_grace_ms:
        normalizeRecoveryPreconfigGraceMs(row.whatsapp_preconfig_message_grace_ms) ??
        WHATSAPP_RECOVERY_SETTING_DEFAULTS.preconfigMessageGraceMs,
      whatsapp_recovery_window_hours:
        normalizeRecoveryWindowHours(row.whatsapp_recovery_window_hours) ??
        WHATSAPP_RECOVERY_SETTING_DEFAULTS.recoveryWindowHours,
      whatsapp_recovery_batch_limit:
        normalizeRecoveryBatchLimit(row.whatsapp_recovery_batch_limit) ??
        WHATSAPP_RECOVERY_SETTING_DEFAULTS.recoveryBatchLimit,
      whatsapp_recovery_analysis_message_limit:
        normalizeRecoveryAnalysisMessageLimit(row.whatsapp_recovery_analysis_message_limit) ??
        WHATSAPP_RECOVERY_SETTING_DEFAULTS.recoveryAnalysisMessageLimit,
      whatsapp_recovery_ai_required:
        typeof row.whatsapp_recovery_ai_required === 'boolean'
          ? row.whatsapp_recovery_ai_required
          : WHATSAPP_RECOVERY_SETTING_DEFAULTS.recoveryAiRequired,
      whatsapp_recovery_ai_model:
        normalizeRecoveryAiModel(row.whatsapp_recovery_ai_model) ??
        WHATSAPP_RECOVERY_SETTING_DEFAULTS.recoveryAiModel,
    };
  } finally {
    connection.release();
  }
...
export async function updateAdminAISettings(
  ...
  try {
    ...
    if (typeof whatsapp_pending_recovery_enabled === 'boolean') {
      updates.push('whatsapp_pending_recovery_enabled = ?');
      values.push(whatsapp_pending_recovery_enabled);
    }
    if (typeof whatsapp_only_post_config_messages === 'boolean') {
      updates.push('whatsapp_only_post_config_messages = ?');
      values.push(whatsapp_only_post_config_messages);
    }
    if (whatsapp_preconfig_message_grace_ms !== undefined) {
      const normalized = normalizeRecoveryPreconfigGraceMs(whatsapp_preconfig_message_grace_ms);
      if (normalized !== null) {
        updates.push('whatsapp_preconfig_message_grace_ms = ?');
        values.push(normalized);
      }
    }
    if (whatsapp_recovery_window_hours !== undefined) {
      const normalized = normalizeRecoveryWindowHours(whatsapp_recovery_window_hours);
      if (normalized !== null) {
        updates.push('whatsapp_recovery_window_hours = ?');
        values.push(normalized);
      }
    }
    if (whatsapp_recovery_batch_limit !== undefined) {
      const normalized = normalizeRecoveryBatchLimit(whatsapp_recovery_batch_limit);
      if (normalized !== null) {
        updates.push('whatsapp_recovery_batch_limit = ?');
        values.push(normalized);
      }
    }
    if (whatsapp_recovery_analysis_message_limit !== undefined) {
      const normalized = normalizeRecoveryAnalysisMessageLimit(
        whatsapp_recovery_analysis_message_limit
      );
      if (normalized !== null) {
        updates.push('whatsapp_recovery_analysis_message_limit = ?');
        values.push(normalized);
      }
    }
    if (typeof whatsapp_recovery_ai_required === 'boolean') {
      updates.push('whatsapp_recovery_ai_required = ?');
      values.push(whatsapp_recovery_ai_required);
    }
    if (whatsapp_recovery_ai_model !== undefined) {
      updates.push('whatsapp_recovery_ai_model = ?');
      values.push(normalizeRecoveryAiModel(whatsapp_recovery_ai_model));
    }
  ...
  try {
    ...
    const [rows] = await connection.query(
      `INSERT INTO catalog_items
       (admin_id, item_type, name, category, description, price_label, duration_value, duration_unit, duration_minutes, quantity_value, quantity_unit, details_prompt, keywords, is_active, sort_order, is_bookable, is_time_based)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
       RETURNING id`,
      [
        adminId,
        ...
        item_type === 'service' ? Boolean(is_time_based) : false,
      ]
    );
...
export async function updateCatalogItem(itemId, adminId, updates = {}) {
  ...
  try {
    ...
    if (Object.prototype.hasOwnProperty.call(updates, 'is_time_based')) {
      fields.push('is_time_based = ?');
      params.push(Boolean(updates.is_time_based));
    }
